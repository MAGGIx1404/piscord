  ...
  <div class="flex w-100 shrink-0 flex-col border-r border-border/50">
    ...
    <div class="flex-1 overflow-y-auto">
        ...
        <div v-else class="space-y-0.5 p-2">
          <template v-if="onlineFriends.length">
            ...
            <FriendsListItem
              ...
              @select="$emit('select-friend', f)"
              @remove="$emit('remove-friend', f)"
            />
          </template>
      ...
      <template v-else-if="activeTab === 'pending'">
        ...
        <div v-else class="space-y-4 p-3">
          <div v-if="incomingRequests.length">
            ...
            <div class="space-y-2">
              <LazyFriendsRequestItem
                v-for="req in incomingRequests"
                :key="req.id"
                :request="req"
                type="incoming"
                @accept="$emit('accept-request', req)"
                @decline="$emit('decline-request', req)"
              />
            </div>
          </div>
          <div v-if="outgoingRequests.length">
            ...
            <div class="space-y-2">
              <LazyFriendsRequestItem
                ...
                :request="req"
                type="outgoing"
                @cancel="$emit('cancel-request', req)"
              />
            </div>
          </div>
        </div>
      ...
      <template v-else-if="activeTab === 'search'">
        ...
        <div v-else class="space-y-0.5 p-2">
          <LazyFriendsSearchResult
            v-for="u in searchResults"
            :key="u.id"
            :user="u"
            @add-friend="$emit('send-request', u)"
          />
        </div>
      </template>
    </div>
...
import { Search as SearchIcon, Users as UsersIcon, Inbox } from "lucide-vue-next";
...
const emit = defineEmits<{
  ...
  "remove-friend": [friend: Friend];
  "accept-request": [req: any];
  "decline-request": [req: any];
  "cancel-request": [req: any];
  "send-request": [user: any];
  search: [query: string];
}>();
...
const tabs = computed(() => [
  { id: "all" as const, label: "All Friends", count: props.friends.length },
  {
    id: "pending" as const,
    label: "Pending",
    count: props.incomingRequests.length + props.outgoingRequests.length
  },
  { id: "search" as const, label: "Search", count: 0 }
]);

const onlineFriends = computed(() => props.friends.filter((f) => f.is_online));
const offlineFriends = computed(() => props.friends.filter((f) => !f.is_online));
